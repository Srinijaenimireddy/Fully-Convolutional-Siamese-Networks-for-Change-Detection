...
Trained on **445 images** → **1827 patches** (after filtering) for **25 epochs** on Kaggle T4 x2 GPU.

| Model | F1 | mIoU | AUC |
|-------|----|------|-----|
| FC-EF | 0.6947 | 0.7086 | 0.9180 |
| FC-Siam-conc | 0.7687 | 0.7669 | 0.9566 |
| FC-Siam-diff | 0.7464 | 0.7485 | 0.9467 |
| **Ensemble (Majority Voting)** | **0.7590** | **0.7597** | **0.9501** |

**Key observations:**
- FC-Siam-conc achieves the best individual model performance with 76.69% mIoU and 76.87% F1
- FC-Siam-diff outperforms FC-EF despite using the same encoder depth, validating the benefit of explicit difference-based skip connections over early fusion
- All models achieve AUC above 0.91, indicating strong ability to distinguish change from no-change pixels across all probability thresholds
- Ensemble majority voting improves over FC-EF and FC-Siam-diff, though FC-Siam-conc remains the strongest — a known limitation of majority voting where a strong model can be outvoted by two weaker ones
- Results achieved with 25 epochs and a subset of the full dataset — longer training on the complete LEVIR-CD training set would approach paper-reported 83-84% mIoU
